...
TEMPLATE_FILE = BASE_DIR / "v8_page_template.html"
...
KEYWORD_CSV = BASE_DIR / "v8_keywords_added.csv"
REPORT_CSV = BASE_DIR / "v8_run_report.csv"
SUMMARY_JSON = BASE_DIR / "v8_run_summary.json"
...
# 사용자가 요청한 핵심 서비스 중심
SERVICES = [
    ("입주청소", "move-in-cleaning", 30),
    ("이사청소", "moving-cleaning", 25),
    ("거주청소", "residential-cleaning", 12),
    ("상가청소", "store-cleaning", 8),
    ("사무실청소", "office-cleaning", 8),
    ("폐기물청소", "waste-cleaning", 5),
    ("원룸청소", "one-room-cleaning", 4),
    ("빌라청소", "villa-cleaning", 3),
    ("오피스텔청소", "officetel-cleaning", 3),
    ("아파트청소", "apartment-cleaning", 2),
]
...
def existing_state() -> tuple[set[str], set[str], list[str]]:
    ...
    for child in BASE_DIR.iterdir():
        ...
        match = title_re.search(content) or h1_re.search(content)
        if match:
            text = re.sub(r"<[^>]+>", " ", match.group(1))
            existing_keywords.add(normalize_keyword(html.unescape(text)))
...
def make_description(page: Candidate) -> str:
    options = (
        f"{page.keyword}를 알아보는 분을 위해 작업 범위, 진행 순서, 견적 상담 시 확인할 사항을 정리했습니다. 체인지클린 청소 안내입니다.",
        f"{page.location} {page.property_type} {page.service} 관련 비용과 작업 범위, 예약 전 확인사항을 안내합니다. 현장 상태에 맞춰 상담받아보세요.",
...
def make_feedback(page: Candidate) -> str:
    options = (
        f"{page.location} {page.property_type} 현장을 확인한 뒤 {page.service} 작업 순서를 정했습니다. "
        "창틀과 수납장 안쪽의 먼지를 먼저 정리하고 주방과 욕실의 생활 오염을 구역별로 세척했습니다. "
        "마지막에는 바닥과 모서리, 손이 자주 닿는 부분까지 다시 확인했습니다.",

        f"{page.keyword} 상담 후 현장 구조와 오염 상태를 먼저 살폈습니다. "
        "같은 평수라도 짐의 유무와 오염 범위에 따라 필요한 장비와 작업 시간이 달라질 수 있어 "
        "공간별로 순서를 나누어 꼼꼼하게 진행했습니다.",

        f"{page.location}에서 진행한 {page.service}는 높은 곳의 먼지 제거부터 시작했습니다. "
        "창틀, 주방, 욕실, 바닥 순서로 작업하고 보이지 않는 수납장 내부와 배수구 주변도 "
        "잔여 오염이 없는지 마무리 점검했습니다.",
    )
...
def make_faq(page: Candidate) -> str:
    items = (
        (
            f"{page.service} 범위는 어디까지인가요?",
            "기본 범위는 실내 먼지와 생활 오염 제거입니다. 외창, 심한 특수오염, 폐기물 반출은 현장에 따라 별도 확인합니다.",
        ),
        (
            "견적을 받으려면 무엇을 알려드려야 하나요?",
            "지역, 공간 종류, 평수, 방과 욕실 개수, 짐과 폐기물 유무, 원하는 날짜를 알려주시면 상담이 빠릅니다.",
        ),
        (
            "주말이나 급한 일정도 가능한가요?",
            "예약 현황에 따라 가능합니다. 원하는 날짜와 입실 또는 이사 시간을 함께 알려주시면 확인해드립니다.",
        ),
    )
    return "\n".join(
...
def load_template() -> str:
    if not TEMPLATE_FILE.exists():
        raise FileNotFoundError(
            f"v8_page_template.html이 없습니다: {TEMPLATE_FILE}"
        )
...
def main() -> int:
    ...
    if not TEMPLATE_FILE.exists():
        log("v8_page_template.html이 없습니다.")
        return 1

    home_hash_before = hashlib.sha256(home.read_bytes()).hexdigest()

    log("=" * 72)
    log("V8 Incremental SEO Generator")
    ...
    try:
        ...
        if checked and not args.allow_check:
            ...
            log("v8_run_summary.json 확인 후 --allow-check로 실행하세요.")
            return 3
        ...
        if args.no_git:
            ...
            message = args.message or (
                f"V8 add {len(created)} longtail pages "
