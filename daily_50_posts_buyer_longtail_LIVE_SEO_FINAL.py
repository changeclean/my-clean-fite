...
def make_article(parent_slug, keyword, topic, post_slug, seq, imgs):
    ...
    intro, sections = article_body(keyword, topic, parent_slug + post_slug)
...
.live-track{{animation:liveScroll 32s linear infinite;will-change:transform;padding-top:0}}
...
@keyframes liveScroll{{0%,8%{{transform:translateY(0)}}92%,100%{{transform:translateY(-520px)}}}}
@media(max-width:600px){{h1{{font-size:27px}}.wrap{{padding:20px 16px}}.live-window{{height:210px}}.live-row{{height:70px;grid-template-columns:58px 1fr 70px 58px;padding:0 10px;font-size:12px}}.live-service{{display:none}}@keyframes liveScroll{{0%,8%{{transform:translateY(0)}}92%,100%{{transform:translateY(-560px)}}}}}}
</style></head><body>
<header><div class="in">
