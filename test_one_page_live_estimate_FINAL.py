...
bak=p.with_name(p.name+".before_fix_test.bak")
...
m=re.search(r'<a\\b[^>]*>[^<]*상담직통[^<]*</a>',s,re.I)
button_ok=False
if m:
    phone=m.group(0)
...
live="""<!-- CHANGE_CLEAN_LIVE_ESTIMATE_FINAL -->
...
print("테스트 파일:",p)
print("실시간 견적문의 최종 블록:",s.count("CHANGE_CLEAN_LIVE_ESTIMATE_FINAL"),"개")
