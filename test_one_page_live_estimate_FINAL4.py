...
# 1) 기존 실시간 견적문의 섹션/빈 중복박스를 모두 제거.
...
# 이전 버전에서 남은 고아(빈) 실시간 견적문의 박스까지 제거
# cc-live-title을 포함한 cc-live-box가 단독으로 남은 경우 제거
s=re.sub(r'<(?:section|div)\b[^>]*class=["\'][^"\']*cc-live-wrap[^"\']*["\'][^>]*>.*?</(?:section|div)>\s*(?:<script[^>]*>.*?</script>)?', '', s, flags=re.I|re.S)
s=re.sub(r'<div\b[^>]*class=["\'][^"\']*cc-live-box[^"\']*["\'][^>]*>\s*<div\b[^>]*class=["\'][^"\']*cc-live-title[^"\']*["\'][^>]*>[^<]*실시간\s*견적문의[^<]*</div>\s*(?:<div\b[^>]*class=["\'][^"\']*cc-live-window[^"\']*["\'][^>]*>.*?</div>)?\s*</div>', '', s, flags=re.I|re.S)
...
live="""<!-- CHANGE_CLEAN_LIVE_ESTIMATE_FINAL4 -->
...
print("테스트 파일:",p)
print("실시간 견적문의 최종 블록:",s.count("CHANGE_CLEAN_LIVE_ESTIMATE_FINAL4"),"개")
